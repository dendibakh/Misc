...
namespace Private
{
   ...
   RestoreFolder::RestoreFolder(const path& binaryFile, const path& folderPath, bool needPrintInfo) : 
                  folderPath(removeLastSlash(folderPath)), binaryFile(removeLastSlash(binaryFile)), binary(nullptr), needPrintInfo(needPrintInfo), filesCount(0), foldersCount(0), size(0)
   {  
      ...
      ifstream binaryHandler(binaryFile.string(), ifstream::binary);
      if (!binaryHandler)
      {
   ...
   void RestoreFolder::printFolderInfo() const
   {
      ...
      ofstream dirInfo(dirInfoFileName, ofstream::binary | ofstream::app);
      try
      {
              if (!dirInfo)
