...
RestoreFile::RestoreFile(ifstream& binary, const boost::filesystem::path& outputDir) : binary(binary), outputDir(removeLastSlash(outputDir))
{
        try
        {
                checkExistFile();
...
{
        ofstream output(outputFilePath.string(), ofstream::binary | ofstream::app);
