...
void HuffmanTrie::expand(compressedResult::const_iterator begin, const compressedResult::const_iterator& end, std::string& result) const
{
	if (root->isLeaf())
	{
		result.append(end - begin, root->symbol);
	}
	else
	{
	        compressedResult::const_iterator iter = begin;
	        while ( iter != end )
	        {
		    expand(root, iter, end, result);
		}
	}
}
...
{
	std::string result;
	expand(comrText.begin(), comrText.end(), result);
	return result;
}
...
HuffmanTrie::compressedResult::const_iterator HuffmanTrie::constructFromBinaryRepresentation(compressedResult::const_iterator begin, compressedResult::const_iterator end)
{
	HuffmanTrie::compressedResult::const_iterator iter = begin;
	constructFromBinaryRepresentation(root, iter, end);
	return iter;
}
