...
void HuffmanAlgorithm::expandTextOnly(compressedResult::const_iterator begin, compressedResult::const_iterator end, std::string& result)
{
        if (begin == end)
		return;

	trie->expand(begin, end, result);
}
...
void HuffmanAlgorithm::getExpandedResult(compressedResult::const_iterator begin, compressedResult::const_iterator end, std::string& result)
{
        trie.reset(new HuffmanTrie(0));
	compressedResult::const_iterator endOfTheTrie = trie->constructFromBinaryRepresentation(begin, end);
	return expandTextOnly(endOfTheTrie, end, result);
}

std::string HuffmanAlgorithm::getExpandedResult(const compressedResult& compressedText)
{
	trie.reset(new HuffmanTrie(0));
	compressedResult::const_iterator endOfTheTrie = trie->constructFromBinaryRepresentation(compressedText.begin(), compressedText.end());
